...
def get_dtable_export_content(username, repo_id, table_name, dtable_uuid, dtable_file_dir_id, asset_dir_id):
    ...
    """
    tmp_file_path = os.path.join('/tmp', dtable_uuid,
                                 'dtable_asset/')  # used to store asset files and json from file_server
    tmp_zip_path = os.path.join('/tmp', dtable_uuid, 'zip_file') + '.zip'  # zip path of zipped xxx.dtable
    ...
    """
    /tmp/<dtable_uuid>/dtable_asset/
                                |- asset/
                                |- content.json

    we zip /tmp/<dtable_uuid>/dtable_asset/ to /tmp/<dtable_id>/zip_file.zip and download it
    notice than make_archive will auto add .zip suffix to /tmp/<dtable_id>/zip_file
    """
    shutil.make_archive('/tmp/' + dtable_uuid +  '/zip_file', "zip", root_dir=tmp_file_path)
