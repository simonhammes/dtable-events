...
class TaskManager:
    ...
    def add_export_task(self, username, repo_id, dtable_uuid, dtable_name):
        ...
        task = threading.Thread(target=get_dtable_export_content, args=(username, repo_id, dtable_name, dtable_uuid,
    ...
    def add_import_task(self, username, repo_id, workspace_id, dtable_uuid, dtable_file_name, uploaded_temp_path):
        ...
        task = threading.Thread(target=post_dtable_import_files, args=(username, repo_id, workspace_id, dtable_uuid, dtable_file_name, uploaded_temp_path))
        task.start()
        task_id = str(int(time.time()*1000))
        self.task_pool[task_id] = task
